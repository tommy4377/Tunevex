...
fn toggle_ifeo_item(id: &str, enable: bool) -> Result<(), String> {
    ...
    if enable {
        ...
        let backup_path = format!(r"Software\Tunevex\Backups\IFEO\{}", exe_name);
    ...
    if let Ok(key) = hklm.open_subkey_with_flags(path, KEY_ALL_ACCESS) {
        if enable {
            ...
            let backup_path = if is_32bit {
                r"Software\Tunevex\Backups\AppInit32"
            } else {
                r"Software\Tunevex\Backups\AppInit64"
            };
            if let Ok(backup_key) = hkcu.open_subkey(backup_path) {
...
fn toggle_bootexec_item(id: &str, enable: bool) -> Result<(), String> {
    ...
    if enable {
        // Restore from backup
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        let backup_path = r"Software\Tunevex\Backups\BootExecute";
        if let Ok(backup_key) = hkcu.open_subkey(backup_path) {
