...
pub fn set_values(owner: &str, values: Vec<(String, String, RegValue)>) -> Result<(), String> {
    if values.is_empty() {
        return Err("No supported devices expose this setting".into());
    }
...
pub fn restore(owner: &str) -> Result<(), String> {
    let file = journal_path(owner)?;
    let saved: BTreeMap<String, SavedValue> =
        serde_json::from_slice(&std::fs::read(&file).map_err(|_| {
            "No saved device values for this tweak; refusing to guess a driver default".to_string()
        })?)
        .map_err(|e| format!("Invalid device backup: {e}"))?;
    if saved.is_empty() {
        return Err("No saved device values to restore".into());
    }
    let root = RegKey::predef(HKEY_LOCAL_MACHINE);
    ...
    for entry in saved.values() {
        ...
        if !can_restore(&current, entry) {
            return Err(format!(
                "{}\\{} changed since apply; revert the overlapping tweak first",
...
mod tests {
    ...
    fn restoration_preserves_absence_and_rejects_overlapping_changes() {
        ...
        assert!(serde_json::from_str::<SavedValue>(&json)
            .unwrap()
            .original
            .is_none());
    }
