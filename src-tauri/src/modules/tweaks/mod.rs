...
pub fn check_tweak_enabled(check: &TweakCheck) -> bool {
    match check {
        ...
        TweakCheck::MsiEnabledForClass { class, priority } => {
            check_msi_enabled_for_class(class, *priority)
        }
        TweakCheck::Registry {
            root_key,
            path,
        ...
        TweakCheck::MultiScheduledTaskDisabled { names } => {
            !names.is_empty() && names.iter().all(|name| check_scheduled_task_disabled(name))
        }
...
pub fn check_tweak_state(check: &TweakCheck) -> Option<bool> {
    match check {
        TweakCheck::Registry {
            ...
            key,
            expected_value,
        } => query_registry_value(root_key, path, key, expected_value),
        TweakCheck::MultiRegistry { checks } => {
            if checks.is_empty() {
                return None;
            }
            let values: Option<Vec<bool>> = checks
                .iter()
                .map(|c| query_registry_value(&c.root_key, &c.path, &c.key, &c.expected_value))
                .collect();
        ...
        TweakCheck::CommandOutputContains {
            ...
            if crate::modules::utils::security::validate_command(cmd).is_err() {
                return None;
            }
        ...
        TweakCheck::NetAdapterProperty {
            property,
            expected_value,
        } => query_nic_property(property, expected_value),
        ...
        TweakCheck::MsiEnabledGlobally { priority } => {
            let states: Vec<bool> = ["Display", "SCSIAdapter", "Net", "USB", "HDC"]
                .iter()
                .filter_map(|c| query_msi_state(c, *priority))
                .collect();
            if states.is_empty() {
                None
            } else {
                Some(states.into_iter().all(|v| v))
            }
        }
        TweakCheck::Powershell {
            script,
        ...
        TweakCheck::MultiScheduledTaskDisabled { names } => {
            ...
            let states: Option<Vec<bool>> = names
                .iter()
                .map(|name| query_scheduled_task_disabled(name))
                .collect();
...
mod regression_tests {
    ...
    fn timestamps_use_netsh_and_heuristics_never_disable_rss() {
        ...
        let timestamps = catalog
            .iter()
            .find(|t| t.id == "net_tcp_1323_opts")
            .unwrap();
        assert!(
            matches!(&timestamps.check, Some(TweakCheck::TcpGlobal { settings })
            if settings == &vec![("timestamps".into(), "enabled".into())])
        );
        for (ops, expected) in [
            (&timestamps.operations, "timestamps=enabled"),
            (
                timestamps.revert_operations.as_ref().unwrap(),
                "timestamps=disabled",
            ),
        ] {
            assert!(ops
                .iter()
                .any(|op| matches!(op, TweakOperation::Command { cmd, args }
                if cmd == "netsh" && args.iter().any(|arg| arg == expected))));
        }
        let heuristics = catalog
            .iter()
            .find(|t| t.id == "net_tcp_heuristics_disable")
            .unwrap();
        ...
        assert!(!serde_json::to_string(&heuristics.revert_operations)
            .unwrap()
            .contains("rss="));
    }
    ...
    fn queryable_examples_are_detectable_and_reversible_where_supported() {
        ...
        for id in [
            "net_tcp_autotuning",
            "net_disable_jumbo_packet",
            "net_optimize_speed",
            "interface_classic_context_menu",
            "mem_fixed_pagefile",
            "sys_reserved_storage_disable",
        ] {
            let tweak = catalog.iter().find(|t| t.id == id).unwrap();
            assert_eq!(tweak.tweak_type, TweakType::Toggle, "{id}");
            assert!(
                tweak.check.is_some() && tweak.revert_operations.is_some(),
                "{id}"
            );
        }
        let display = catalog
            .iter()
            .find(|t| t.id == "display_max_refresh_rate")
            .unwrap();
    ...
    fn touched_powershell_scripts_parse_without_executing_tweaks() {
        ...
        for id in [
            ...
            "net_disable_teredo",
            "net_tcp_heuristics_disable",
            "sys_reserved_storage_disable",
        ] {
            let tweak = catalog.iter().find(|t| t.id == id).unwrap();
            let mut scripts = Vec::new();
            if let Some(TweakCheck::Powershell { script, .. }) = &tweak.check {
                scripts.push(script);
            }
            for op in tweak
                .operations
                .iter()
                .chain(tweak.revert_operations.iter().flatten())
            {
                if let TweakOperation::Powershell { script } = op {
                    scripts.push(script);
                }
            }
            for script in scripts {
                ...
                assert!(
                    check_powershell_output(&parse, "True"),
                    "{id} contains invalid PowerShell"
                );
            }
        }
    }
