...
pub fn apply_tweak(
    ...
    println!("[TWEAK APPLY] Applying ID: {}", id);
    for (i, op) in tweak.operations.iter().enumerate() {
        println!(
            "[TWEAK APPLY] Operation {}/{}: {:?}",
            i + 1,
            tweak.operations.len(),
            op
        );
        match op {
    ...
    // 3. Update State & Persist
    {
        ...
        app_state.applied_tweaks.insert(id);
